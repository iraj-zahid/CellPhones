...
function BoxOne(props) {
    ...
    const filteredValue = data.filter((e) => {
        return e.title.toLowerCase().includes(props.allProp.filterValue)
    })
    const filterRate = filteredValue.filter((e) => {
        return e.rating >= props.allProp.rate
    })
    ...
    return (
        <>
            <><div className="mt-8 w-[80%] h-min-[500px]  flex justify-center items-center flex-wrap ml-[120px]">
                {props.allProp.rateTrue ? filterRate && filterRate.map((product) => {
                    return (
                        <>
                            <div onClick={() => {
                                ...
                                navigate("/productDetail", { state: { rating: product.rating * product.price } })
